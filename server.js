...
const predictionSchema = new mongoose.Schema({
  ...
  amount: { type: Number, required: true }, // This will be in USD
  deliveryTime: { type: Number, required: true },
  currentPrice: { type: Number, required: true }, // Price of one unit of the cryptocurrency in USD
...
const sendSchema = new mongoose.Schema({
  ...
  status: { type: String, default: "pending" }, // 'pending', 'complete', 'incomplete'
});
    ...
    const response = await axios.get(
      "https://api.coingecko.com/api/v3/coins/markets",
      {
        params: {
          ...
          per_page: 250, // Fetch up to 250 cryptocurrencies
          page: 1,
          sparkline: true,
        },
      }
...
const evaluatePrediction = async (predictionId, interestRate) => {
  ...
  if (
    ...
    const cryptoProfit = profit / newPrice; // Calculate profit in cryptocurrency
    evalResult = {
      success: true,
      profit,
...
app.post("/api/prediction/:id/result", async (req, res) => {
  ...
  try {
    ...
    const profit = success
      ...
      : 0;
    const cryptoProfit = profit / prediction.currentPrice; // Calculate profit in cryptocurrency
    const result = {
      success,
...
app.get("/api/prediction/:id", async (req, res) => {
  try {
    ...
    console.error("Error fetching prediction result:", error); // Detailed error logging
    res.status(500).json({ error: error.message });
  }
});
